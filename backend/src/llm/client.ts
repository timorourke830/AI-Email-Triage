...
export class LLMClient {
  ...
  async rankEmails(
  ...
  ): Promise<{ result: RankingResult | null; error: string | null }> {
    ...
    const prompt = fillTemplate(RANKING_PROMPT, { emails: emailsText });

    try {
      const response = await this.complete(prompt);
      const parsed = parseAndValidate(response, RankingSchema);
      return { result: parsed.data, error: parsed.error };
    } catch (err) {
      return {
        result: null,
